...
> [!TIP]
> 如果你使用多个浏览器或多个设备，通常需要在执行审批的**每个环境**中分别绑定 Passkey。比如，若需要在[移动端进行审批](#移动端审批可选)，则需要在移动端浏览器登录后，将移动端 Passkey 绑定到 Signoff。若你的凭据提供方支持跨浏览器/跨设备同步，可能无需重复绑定即可使用。

添加完成后，你应能在列表中看到该 authenticator（含使用次数和创建时间）。

新用户注册后，Signoff 会自动绑定一个内置起步规则，供 `signoff quick-start` 使用。你之后可以通过它来体验“保护 -> 审批 -> 重试”的完整旅程，无需先手动创建规则。
